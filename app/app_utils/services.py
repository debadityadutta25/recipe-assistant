...
def get_memory_service():
    """Process-wide memory service: Vertex AI Memory Bank when engine ID set, else in-memory."""
    if memory_bank_id := os.environ.get("MEMORY_BANK_ID") or os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID"):
        from google.adk.memory.vertex_ai_memory_bank_service import VertexAiMemoryBankService

        return VertexAiMemoryBankService(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT") or "qwiklabs-gcp-03-2b2a39607757",
            location=os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION")
            or os.environ.get("GOOGLE_CLOUD_LOCATION")
            or "us-east1",
